...
- **Minimal Perspective Autocalibration**  
  Introduces a new family of minimal autocalibration problems and practical solvers for recovering camera intrinsics from point correspondences. {% cite %}

- **Revisiting Calibration of Wide-Angle Radially Symmetric Cameras**  
  Proposes a two-step framework that separates an implicit camera representation from the final parametric model, improving flexibility in wide-angle calibration. {% cite %}
...
- **One Target to Align Them All: LiDAR, RGB and Event Cameras Extrinsic Calibration**  
  Presents a joint calibration framework for heterogeneous sensor suites, with particular attention to the challenges posed by event cameras. {% cite %}
...
Rather than focusing on a single application area, the common thread is the development of methods for settings where structure, ambiguity, and noise play a central role.